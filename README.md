...
从 **F-Droid** 安装以下两个 App（不要用 Play Store 版本）：

- [Termux](https://f-droid.org/packages/com.termux/) — 终端模拟器
- [Termux:API](https://f-droid.org/packages/com.termux.api/) — 让脚本访问手机硬件
...
```bash
pkg install -y git && git clone https://github.com/Osgood001/termux-claude-setup.git && bash termux-claude-setup/install.sh
```

安装脚本会自动完成：
- 安装所有依赖（Node.js、Python、cronie、proot、termux-api...）
- 安装 Claude Code 并配置 /tmp 修复
- 引导你配置 API（支持 Anthropic 官方 / [LuckyAPI 国内代理](https://cn.luckyapi.chat)）
- 复制示例脚本到 `~/scripts/`
- 生成 CLAUDE.md 人格模板
- 可选安装示例 crontab
...
```bash
source ~/.bashrc         # 使配置生效
nano ~/.claude/CLAUDE.md # 定义你的 AI 人格
claude                   # 开始对话
crontab -e               # 自定义定时任务
```
...
**Q: Claude Code 收费吗？**
A: Claude Code 需要 API key，按用量收费。安装脚本支持两种来源：Anthropic 官方 或 [LuckyAPI 国内代理](https://cn.luckyapi.chat)（官方价格 6%，国内直连免翻墙）。纯通知回退模式不消耗 API 额度。合理配置 cron 频率后月费用极低。
